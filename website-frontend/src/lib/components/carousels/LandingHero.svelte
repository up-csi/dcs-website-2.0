		...
		<Carousel.Content>
			{#each news as news_item, index}
				<Carousel.Item class="relative flex h-[90vh] flex-col items-center justify-end">
					...
					{/if}
					<div
						class="absolute flex h-full w-full flex-col justify-end bg-gradient-to-t from-[#0000008e] to-transparent"
					>
						<div
							class="container max-w-3xl mx-auto flex flex-col items-center gap-y-5 pb-14 pt-72 text-center text-secondary-foreground md:items-start md:text-left md:mx-0"
							on:mouseenter={stopAutoplay}
							on:mouseleave={resetAutoplay}
						>
							{#if index + 1 === current}
								<div transition:fly={{ x: 40, duration: 1000, easing: cubicOut }}>
									<h1 class="text-2xl font-bold md:text-4xl drop-shadow-lg">{news_item.title}</h1>
								</div>

								<div transition:fly={{ x: 40, duration: 900, easing: cubicOut }}>
									<p class="text-xs font-medium md:text-base drop-shadow-lg line-clamp-3">{news_item.summary}</p>
								</div>

								<div transition:fly={{ x: 40, duration: 700, easing: cubicOut }}>
									<small class="text-xs md:text-sm drop-shadow-lg">
										by {news_item.user_created.first_name} {news_item.user_created.last_name}
									</small>
								</div>

								<div transition:fly={{ x: 40, duration: 500, easing: cubicOut }}>
							...
							<!-- Pagination -->
							<div class="flex gap-x-3 text-sm font-semibold text-secondary-foreground md:ml-1">
